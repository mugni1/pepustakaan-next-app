import Container from "@/components/Admin/Container";
import MainContainer from "@/components/Admin/MainContainer";
import React from "react";

export default async function page({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  return (
    <MainContainer>
      <Container>
        <section className="flex justify-between mb-4 items-center">
          <h1 className="text-xl poppins-bold">ID : {id}</h1>
          <h1 className="text-green-500 bg-green-200 py-1 px-5 text-xl poppins-semibold rounded-lg">
            DiKembalikan
          </h1>
        </section>
        <hr />
        {/* book  */}
        <section className="my-4 flex gap-4 w-full">
          <div className="h-72 w-48 bg-emerald-300 rounded-xl "></div>
          <div className="w-full flex flex-col">
            <h1 className="poppins-bold text-2xl line-clamp-1">Title</h1>
            <table className="w-full max-w-auto">
              <tbody className="w-full">
                {/* penulis  */}
                <tr className="w-full">
                  <td className="poppins-semibold flex items-center gap-1 pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M201.54,54.46A104,104,0,0,0,54.46,201.54,104,104,0,0,0,201.54,54.46ZM88,192a16,16,0,0,1,32,0v23.59a88,88,0,0,1-32-9.22Zm48,0a16,16,0,0,1,32,0v14.37a88,88,0,0,1-32,9.22Zm-28.73-56h41.46l11.58,25.1A31.93,31.93,0,0,0,128,170.87a31.93,31.93,0,0,0-32.31-9.77Zm7.39-16L128,91.09,141.34,120Zm75.56,70.23c-2,2-4.08,3.87-6.22,5.64V176a7.91,7.91,0,0,0-.74-3.35l-48-104a8,8,0,0,0-14.52,0l-48,104A7.91,7.91,0,0,0,72,176v19.87c-2.14-1.77-4.22-3.64-6.22-5.64a88,88,0,1,1,124.44,0Z"></path>
                    </svg>
                    Penulis
                  </td>
                  <td className="w-10/12">: Penulis</td>
                </tr>
                {/* end penulis  */}
                {/* Penerbit  */}
                <tr>
                  <td className="poppins-semibold flex items-center gap-1  pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,16a87.5,87.5,0,0,1,48,14.28V74L153.83,99.74,122.36,104l-.31-.22L102.38,90.92A16,16,0,0,0,79.87,95.1L58.93,126.4a16,16,0,0,0-2.7,8.81L56,171.44l-3.27,2.15A88,88,0,0,1,128,40ZM62.29,186.47l2.52-1.65A16,16,0,0,0,72,171.53l.21-36.23L93.17,104a3.62,3.62,0,0,0,.32.22l19.67,12.87a15.94,15.94,0,0,0,11.35,2.77L156,115.59a16,16,0,0,0,10-5.41l22.17-25.76A16,16,0,0,0,192,74V67.67A87.87,87.87,0,0,1,211.77,155l-16.14-14.76a16,16,0,0,0-16.93-3l-30.46,12.65a16.08,16.08,0,0,0-9.68,12.45l-2.39,16.19a16,16,0,0,0,11.77,17.81L169.4,202l2.36,2.37A87.88,87.88,0,0,1,62.29,186.47ZM185,195l-4.3-4.31a16,16,0,0,0-7.26-4.18L152,180.85l2.39-16.19L184.84,152,205,170.48A88.43,88.43,0,0,1,185,195Z"></path>
                    </svg>
                    Penerbit
                  </td>
                  <td>: Penerbit</td>
                </tr>
                {/* end Penerbit  */}
                {/* Tahun Terbit  */}
                <tr>
                  <td className="poppins-semibold flex items-center gap-1  pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM72,48v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80H48V48ZM208,208H48V96H208V208Zm-96-88v64a8,8,0,0,1-16,0V132.94l-4.42,2.22a8,8,0,0,1-7.16-14.32l16-8A8,8,0,0,1,112,120Zm59.16,30.45L152,176h16a8,8,0,0,1,0,16H136a8,8,0,0,1-6.4-12.8l28.78-38.37A8,8,0,1,0,145.07,132a8,8,0,1,1-13.85-8A24,24,0,0,1,176,136,23.76,23.76,0,0,1,171.16,150.45Z"></path>
                    </svg>
                    Tahun Terbit
                  </td>
                  <td>: Tahun Terbit</td>
                </tr>
                {/* end Tahun Terbit  */}
                {/* Kategori  */}
                <tr>
                  <td className="poppins-semibold flex items-center gap-1 pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M243.31,136,144,36.69A15.86,15.86,0,0,0,132.69,32H40a8,8,0,0,0-8,8v92.69A15.86,15.86,0,0,0,36.69,144L136,243.31a16,16,0,0,0,22.63,0l84.68-84.68a16,16,0,0,0,0-22.63Zm-96,96L48,132.69V48h84.69L232,147.31ZM96,84A12,12,0,1,1,84,72,12,12,0,0,1,96,84Z"></path>
                    </svg>
                    Kategori
                  </td>
                  <td>: Kategori</td>
                </tr>
                {/* end Kategori  */}
                {/* Stok  */}
                <tr>
                  <td className="poppins-semibold flex items-center gap-1  pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M231.65,194.55,198.46,36.75a16,16,0,0,0-19-12.39L132.65,34.42a16.08,16.08,0,0,0-12.3,19l33.19,157.8A16,16,0,0,0,169.16,224a16.25,16.25,0,0,0,3.38-.36l46.81-10.06A16.09,16.09,0,0,0,231.65,194.55ZM136,50.15c0-.06,0-.09,0-.09l46.8-10,3.33,15.87L139.33,66Zm6.62,31.47,46.82-10.05,3.34,15.9L146,97.53Zm6.64,31.57,46.82-10.06,13.3,63.24-46.82,10.06ZM216,197.94l-46.8,10-3.33-15.87L212.67,182,216,197.85C216,197.91,216,197.94,216,197.94ZM104,32H56A16,16,0,0,0,40,48V208a16,16,0,0,0,16,16h48a16,16,0,0,0,16-16V48A16,16,0,0,0,104,32ZM56,48h48V64H56Zm0,32h48v96H56Zm48,128H56V192h48v16Z"></path>
                    </svg>
                    Stok :
                  </td>
                  <td className="rounded-full poppins-semibold text-purple-600">
                    : 205
                  </td>
                </tr>
                {/* end Stok  */}
                {/* Description  */}
                <tr>
                  <td className="poppins-semibold flex items-center gap-1 pb-1">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      fill="#000000"
                      viewBox="0 0 256 256"
                    >
                      <path d="M232,48H160a40,40,0,0,0-32,16A40,40,0,0,0,96,48H24a8,8,0,0,0-8,8V200a8,8,0,0,0,8,8H96a24,24,0,0,1,24,24,8,8,0,0,0,16,0,24,24,0,0,1,24-24h72a8,8,0,0,0,8-8V56A8,8,0,0,0,232,48ZM96,192H32V64H96a24,24,0,0,1,24,24V200A39.81,39.81,0,0,0,96,192Zm128,0H160a39.81,39.81,0,0,0-24,8V88a24,24,0,0,1,24-24h64ZM160,88h40a8,8,0,0,1,0,16H160a8,8,0,0,1,0-16Zm48,40a8,8,0,0,1-8,8H160a8,8,0,0,1,0-16h40A8,8,0,0,1,208,128Zm0,32a8,8,0,0,1-8,8H160a8,8,0,0,1,0-16h40A8,8,0,0,1,208,160Z"></path>
                    </svg>
                    Description
                  </td>
                  <td>
                    <span className="line-clamp-5">
                      : Lorem ipsum dolor sit amet consectetur adipisicing elit.
                      A vel magni culpa placeat asperiores facilis modi
                      inventore vitae nihil ea. Lorem ipsum dolor sit amet
                      consectetur adipisicing elit. Culpa sint temporibus
                      voluptatum sed iusto, inventore unde deserunt adipisci
                      ipsum, laborum itaque incidunt tenetur at iste earum
                      aliquam? Deserunt facilis dolor laboriosam, modi et dicta
                      totam! Neque, beatae ipsam quod explicabo inventore velit
                      labore cum? Dolore necessitatibus illum illo eum at. Lorem
                      ipsum dolor sit amet consectetur adipisicing elit.
                      Cupiditate, ad! Lorem ipsum dolor, sit amet consectetur
                      adipisicing elit. Illo dolorem quas velit aliquam odio
                      accusantium dicta minus et cum mollitia?
                    </span>
                  </td>
                </tr>
                {/* end Description  */}
              </tbody>
            </table>
          </div>
        </section>
        {/* end book  */}
        {/* user detil  */}
        <section className="w-full flex flex-col">
          <h2 className="font-semibold text-xl poppins-semibold mb-1">
            Detail Peminjam
          </h2>
          <table className="border rounded-lg border-slate-600 w-full">
            <thead>
              <tr className="bg-purple-300 w-full">
                <th className="py-3 w-3/12">Nama Lengkap</th>
                <th className="w-3/12">Username</th>
                <th className="w-3/12">Email</th>
                <th className="w-3/12">Role</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td className="text-center py-3 w-3/12">Nama Lengkap</td>
                <td className="text-center w-3/12">Username</td>
                <td className="text-center w-3/12">Email</td>
                <td className="text-center w-3/12">Role</td>
              </tr>
            </tbody>
          </table>
        </section>
        {/* end user detail  */}
      </Container>
    </MainContainer>
  );
}
